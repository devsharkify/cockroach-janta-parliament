...
export async function GET(req: NextRequest) {
  ...
  const pageViews = BASE_VIEWS + Math.floor(hoursElapsed * VIEWS_PER_HOUR)
  ...
  if (!process.env.MONGODB_URI) {
    return Response.json({
      ...
      currentCycle: {
        cycleNumber: 3,
        status: 'live',
        snapshotAt: new Date(Date.now() + 2 * 24 * 3600 * 1000).toISOString(),
  ...
  try {
    ...
    return Response.json({
      ...
      votesByParty, candidatesByState, xpDistribution, topSeats,
    })
  } catch (err) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (winners.length > 0) {
      ...
      await resultsCol.bulkWrite(ops)
    }
    ...
    const nextCycle = {
      ...
      starts_at: new Date(),
      snapshot_at: nextSun,
      ends_at: new Date(nextSun.getTime() + 60 * 60 * 1000),
