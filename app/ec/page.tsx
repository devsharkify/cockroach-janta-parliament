...
import type { ECPosition } from '@/lib/ecPositions'

const MAX_VOTES = 1000
const LS_VOTES_KEY = 'ec_votes_used'

interface PositionWithCount extends ECPosition {
...
export default function ECPage() {
  ...
  const openPosition = useCallback(async (pos: ECPosition) => {
    ...
    setFormError(null)
    try {
      const res = await fetch(`/api/ec?position_id=${pos.id}`)
  ...
  async function handleVote(app: Application) {
    ...
    setVotingId(app.id)
    try {
      const res = await fetch('/api/ec/vote', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="min-h-screen font-sans" style={{ backgroundColor: '#001a00' }}>
      {/* Nav */}
      <nav className="sticky top-0 z-50 border-b-4 border-black" style={{ backgroundColor: '#001a00' }}>
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-4">
          <a href="/" className="text-yellow-300 font-black text-sm hover:text-yellow-200 transition-colors">
            ← PARLIAMENT
          </a>
          <span className="text-white/20">|</span>
          <span className="text-white font-black text-sm tracking-widest uppercase">Cockroach Election Commission</span>
        </div>
      </nav>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-white mb-2 tracking-tight uppercase">
            COCKROACH ELECTION COMMISSION
          </h1>
          <p className="font-mono text-lg" style={{ color: '#D4A017' }}>
            Fair. Transparent. Completely chaotic.
          </p>
        ...
        {loadingPositions ? (
          <div className="text-center py-20">
            ...
            <p className="text-white/50 font-mono">Loading EC positions...</p>
          </div>
        ) : (
          <>
            {/* NATIONAL COMMISSION */}
            <section className="mb-10">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="font-black text-white text-xl uppercase tracking-widest">National Commission</h2>
                <div className="h-1 flex-1 bg-yellow-300/30" />
              </div>
            ...
            <section>
              <div className="flex items-center gap-3 mb-4">
                <div className="h-1 flex-1 bg-yellow-300/30" />
                <h2 className="font-black text-white text-xl uppercase tracking-widest">State Commissions</h2>
                <div className="h-1 flex-1 bg-yellow-300/30" />
              </div>
              <div className="grid gap-4 md:grid-cols-2">
...
function ECPositionCard({
  ...
  return (
    ...
    >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <span
                ...
                style={{ backgroundColor: pos.level === 'national' ? '#D4A017' : '#16a34a', color: '#000' }}
              >
      ...
      {/* Expanded panel */}
      {isOpen && (
        <div className="border-t-4 border-black p-4 space-y-4" style={{ backgroundColor: '#001a00' }}>
          {!active!.applied ? (
            <>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Ballot Box Bhattacharya"
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. I will ensure every vote is counted, at least once"
                    className="w-full bg-black/40 border-2 border-white/20 focus:border-yellow-300 rounded-xl px-4 py-3 text-white font-mono text-sm placeholder:text-white/20 focus:outline-none transition-colors"
                  />
          ...
          <div className="border-t border-white/10 pt-3 text-center">
            <p className="text-white/30 text-xs font-mono">
              {votesUsed}/{maxVotes} EC votes used
            </p>
            {votesUsed >= maxVotes && (
              <p className="text-red-400 text-xs font-mono mt-1 font-black">All EC votes exhausted!</p>
            )}
          </div>
        </div>
      )}
