...
import { courtApplications, courtVotes } from '@/lib/mongodb/collections'
...
export async function POST(req: NextRequest) {
  ...
  const votesCol = await courtVotes()

  // Count total votes this fingerprint has cast across ALL court positions
  const totalVotesUsed = await votesCol.countDocuments({ voter_fingerprint: String(fingerprint) })

  if (totalVotesUsed >= MAX_VOTES) {
    return Response.json(
      { error: 'You have used all 1000 court votes', votesUsed: totalVotesUsed, votesRemaining: 0 },
      { status: 429 }
    )
  }
  ...
  const appsCol = await courtApplications()
