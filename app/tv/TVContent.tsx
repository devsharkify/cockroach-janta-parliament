...
import { CATEGORY_COLORS, type NewsArticle } from '@/lib/newsArticles'
...
function getCover(index: number) { return COVER_IMAGES[index % COVER_IMAGES.length] }
  ...
  return (
    <article className="border-4 border-black rounded-2xl bg-white shadow-[5px_5px_0_black] overflow-hidden mb-6">
      ...
      <div className="px-6 pt-6 pb-5 border-b-4 border-black" style={{ background: `${CATEGORY_COLORS[article.category]}12` }}>
        ...
        {imgSrc && article.isFeatured && (
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[9px] font-black bg-yellow-300 text-black px-2 py-0.5 rounded-sm border-2 border-black">⭐ FEATURED</span>
          </div>
        )}
...
/* ── Main content ────────────────────────────────────────────────────────── */
// Set your YouTube live stream / video ID here, e.g. 'dQw4w9WgXcQ'
...
export default function TVContent({ articles }: { articles: NewsArticle[] }) {
  const featured = articles[0]
  const top3 = articles.slice(1, 4)
  const grid6 = articles.slice(4, 10)
  const remaining = articles.slice(10, 20)
  const sidebar = articles.filter(a => a.isHot).slice(0, 5)
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F0]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <aside className="space-y-6">
            ...
            <div className="border-4 border-black rounded-xl bg-black text-white shadow-[4px_4px_0_black] p-4">
              ...
              {[
                { label: 'Stories published', value: `${articles.length}` },
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-6">
          <div className="flex-1 h-1 bg-black" />
          <span className="font-black text-xs uppercase tracking-widest px-3">Be a CJTV Journalist</span>
          <div className="flex-1 h-1 bg-black" />
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          <div className="flex flex-col gap-5">
            <div className="border-4 border-black rounded-2xl bg-yellow-300 shadow-[6px_6px_0_black] p-6">
              <div className="text-5xl mb-3">🪳📰</div>
              <h2 className="font-black text-2xl uppercase leading-tight text-black mb-2">
                You have a story.<br />CJTV has a naali.
              </h2>
              <p className="font-mono text-sm text-black/70 leading-relaxed mb-4">
                Spotted a suspicious broom? Onion prices gone feral? Local dog running for office? Submit your headline and report — we&apos;ll generate your journalist identity on the spot.
              </p>
              <ul className="space-y-2">
                {[
                  'No journalism degree required (roach spirit is enough)',
                  'Get an auto-generated CJTV journalist identity',
                  'All reports are sacred satire, nothing real',
                  'Submit unlimited reports — democracy never sleeps',
                ].map((item, i) => (
                  <li key={i} className="flex items-start gap-2 font-mono text-xs text-black/80">
